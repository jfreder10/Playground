...
#put the data in some folder and then change the directory below to it
...
#define function that will create equal to expand grid in R (gets all combos of input parameters)
def expandgrid(*itrs):
...
for i in range(0,len(params)):
    scores=cross_val_score(RandomForestClassifier( random_state=0, n_estimators=params.iloc[i,0],
...
estValRmse=list(range(0,len(params)))
for i in range(0,len(params)):
    scores=cross_val_score(RandomForestRegressor( random_state=0, n_estimators=params.iloc[i,0],
